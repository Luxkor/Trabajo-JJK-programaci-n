...
app.get('/server-info', (_req, res) => {
  res.json({ ips: getLocalAddresses(), port: PORT });
});
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`╠══════════════════════════════════════════════════════════╣`);
  console.log(`║  📍 Mismo PC   ▶  http://localhost:${PORT}                  ║`);
  if (localAddrs.length) {
    localAddrs.forEach(addr => {
      const url = `http://${addr}:${PORT}`;
      console.log(`║  🌐 Otro PC    ▶  ${pad(url, 40)}║`);
    });
    console.log(`╠══════════════════════════════════════════════════════════╣`);
    console.log(`║  ⬆  El jugador 2 abre esa URL en su navegador.          ║`);
    console.log(`║  ⬆  Deja el campo de servidor vacío (se auto-rellena).  ║`);
  } else {
    console.log(`║  ⚠️  No se detectaron IPs locales. Comprueba red/VPN.   ║`);
  }
  console.log(`╚══════════════════════════════════════════════════════════╝\n`);
